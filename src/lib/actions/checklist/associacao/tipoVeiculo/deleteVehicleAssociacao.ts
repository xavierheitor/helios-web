...
export async function deleteChecklistVehicleTypeAssociation(
  id: number
): Promise<ActionResult> {
  logger.info(
    `deleteChecklistVehicleTypeAssociation action called for id ${id}`
  );
  ...
  try {
    ...
    await prisma.checklistVehicleTypeAssociation.delete({
      where: { id },
    });
